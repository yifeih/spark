...
class DAGSchedulerSuite extends SparkFunSuite with LocalSparkContext with TimeLimits {
  ...
  test("All shuffle files on the slave should be cleaned up when slave lost") {
    ...
    complete(taskSets(2), Seq(
      (FetchFailed(DefaultMapShuffleLocations.get(BlockManagerId("exec-hostA2", "hostA", 12345)),
  ...
  test("run trivial shuffle with fetch failure") {
    ...
    complete(taskSets(1), Seq(
      (Success, 42),
      (FetchFailed(makeShuffleLocation("hostA"), shuffleId, 0, 0, "ignored"), null)))
  ...
  private def completeNextStageWithFetchFailure(
    ...
    complete(stageAttempt, stageAttempt.tasks.zipWithIndex.map { case (task, idx) =>
      (FetchFailed(makeShuffleLocation("hostA"), shuffleDep.shuffleId, 0, idx, "ignored"), null)
    }.toSeq)
  }
  ...
  test("trivial shuffle with multiple fetch failures") {
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(makeShuffleLocation("hostA"), shuffleId, 0, 0, "ignored"),
      null))
    sc.listenerBus.waitUntilEmpty(WAIT_TIMEOUT_MILLIS)
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(makeShuffleLocation("hostA"), shuffleId, 1, 1, "ignored"),
      null))
  ...
  test("late fetch failures don't cause multiple concurrent attempts for the same map stage") {
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(1),
      FetchFailed(makeShuffleLocation("hostB"), shuffleId, 1, 1, "ignored"),
      null))
  ...
  test("stage used by two jobs, some fetch failures, and the first job no longer active " +
    ...
    complete(taskSets(1), Seq(
      (FetchFailed(makeShuffleLocation("hostA"), shuffleDep1.shuffleId, 0, 0, "ignored"), null)))
    scheduler.resubmitFailedStages()
  ...
  test("recursive shuffle failures") {
    ...
    complete(taskSets(2), Seq(
      (FetchFailed(makeShuffleLocation("hostA"), shuffleDepTwo.shuffleId, 0, 0, "ignored"), null)))
    // TODO assert this:
    // blockManagerMaster.removeExecutor("exec-hostA")
  ...
  test("map stage submission with multiple shared stages and failures") {
    ...
    complete(taskSets(1), Seq(
      ...
      (FetchFailed(makeShuffleLocation("hostA"), dep1.shuffleId, 0, 0, "ignored"), null)))
    scheduler.resubmitFailedStages()
    ...
    complete(taskSets(4), Seq(
      (Success, 52),
      (FetchFailed(makeShuffleLocation("hostD"), dep2.shuffleId, 0, 0, "ignored"), null)))
    scheduler.resubmitFailedStages()
      ...
      rdd1.map {
        case (x, _) if (x == 1) =>
          throw new FetchFailedException(
            toShuffleLocation(BlockManagerId("1", "1", 1)), shuffleHandle.shuffleId, 0, 0, "test")
        case (x, _) => x
      }.count()
    }
  ...
  test("[SPARK-19263] DAGScheduler should not submit multiple active tasksets," +
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(makeShuffleLocation("hostA"), shuffleIdA, 0, 0,
  ...
  test("SPARK-23207: retry all the succeeding stages when the map stage is indeterminate") {
    ...
    runEvent(makeCompletionEvent(
      taskSets(2).tasks(0),
      FetchFailed(makeShuffleLocation("hostC"), shuffleId2, 0, 0, "ignored"),
      null))
    ...
    runEvent(makeCompletionEvent(
      taskSets(3).tasks(0),
      FetchFailed(makeShuffleLocation("hostA"), shuffleId1, 0, 0, "ignored"),
      null))
...
object DAGSchedulerSuite {
  ...
  def makeShuffleLocation(host: String): DefaultMapShuffleLocations = {
    DefaultMapShuffleLocations.get(makeBlockManagerId(host))
  }

  def makeMaybeShuffleLocation(host: String): Option[DefaultMapShuffleLocations] = {
    Some(DefaultMapShuffleLocations.get(makeBlockManagerId(host)))
  }

  def toShuffleLocation(blockManagerId: BlockManagerId): ShuffleLocation = {
